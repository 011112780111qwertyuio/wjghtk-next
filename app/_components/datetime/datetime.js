...
function Datetime({ funItems, uid, local, funDataFormation, clearAllData }) {
  ...
  const touchStartX = useRef(0);  // Use ref to store the touch start X position
  const touchEndX = useRef(0);    // Use ref to store the touch end X position
  ...
  const handleTouchEnd = () => {
    if (touchStartX.current - touchEndX.current > 50) {
      funButton("right"); // Swipe left to go to the next month
    }
    if (touchEndX.current - touchStartX.current > 50) {
      funButton("left"); // Swipe right to go to the previous month
    }
  };
