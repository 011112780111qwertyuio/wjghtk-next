...
function AddNews() {
  ...
  const getData = async () => {
    const url = await `${myvalue.nameLocal}/${process.env.NEXT_PUBLIC_API_KEY_ADDTOPICK}`;
    if (dataAll.area.trim().length === 0 || dataAll.area === "") {
      ...
      await axios.post(url, obj).then((res) => {
        if (res.data === "ok-insert") {
          ...
          alert('حدث خطاء الرجاء المحاولة مرة اخرى')
        }
      })
      setloading(false)
    }
            ...
            <div className="flex-bwtween">
              ...
              <div className="right">
                ...
                <div className="child-1 child-card">
                  ...
                  <select onChange={handelData} defaultValue={dataAll.name} name="name">
