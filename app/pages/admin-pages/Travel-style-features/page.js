...
function TravelStyleFeatures() {
  ...
  const getData = async () => {
    const url = await `${myvalue.nameLocal}/${process.env.NEXT_PUBLIC_API_KEY_AddTRAVELSTYLEFEATURES}`;
    setloading(true);
    if (values.myname !== "") {
  ...
  const dlelte = async (val) => {
    setloading(true);
    const url = await `${myvalue.nameLocal}/${process.env.NEXT_PUBLIC_API_KEY_DELETETRAVELSTYLEFEATURES}`;
    const obj = new FormData();
    obj.append("myid", val);
    await axios.post(url, obj).then((res) => {
      if (res.data === "ok") {
          ...
          <div className="flex-betwen">
            ...
            <div className="right-content">
              <div className="content-from">
                <NavparTopContent t1={"رحلة"} t2={"سمات"} t3={"انماط السفر"} />
                <br></br>
                  ...
                  <div className="flex-space-between">
                    <div className="left-from">
                      ...
                      <div className="padd">
                        <label>اسم</label>
                        <br></br>
                        <input name="myname" value={values.myname} onChange={handelValues} type="text" placeholder="اسم السمة مثال :  الطبيعة والمغامرة" />
                        <br></br>
                        <br></br>
                          ...
                          <tbody>
                            {alldata.map((item, index) => (
                              <tr key={index}>
                                ...
                                <td>{item.DtTime}</td>
                                <td onClick={() => { dlelte(item.ID_travel_style_features) }} className="delete">حذف</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
...
export default TravelStyleFeatures;